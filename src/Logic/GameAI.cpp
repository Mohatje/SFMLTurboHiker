...
GameAI& GameAI::getInstance() { return sharedInstance; }
...
std::vector<std::shared_ptr<Entity>> GameAI::getEntitiesInSight(std::shared_ptr<Entity>& hiker)
{
        ...
        for (auto& entity : _world.lock()->getEntities()) {
                ...
                double tmpDistance =
                    sqrt(pow((curPosition.first - entPos.first), 2) + pow((curPosition.second - entPos.second), 2));
...
void GameAI::_updateMovingHiker(std::shared_ptr<Entity>& hiker, float dTime)
{
        ...
        for (auto& ent : toObserve) {
                if (ent->getType() != EntityType::StaticHikerActive && ent->getType() != EntityType::MovingHikerActive)
                        continue;
                if (ent->getPosition().second > onePos.second)
                        continue;
...
void GameAI::_updateRacer(std::shared_ptr<Entity>& hiker, float dTime)
{
        ...
        for (auto& ent : toObserve) {
                ...
                if (!(ent->getType() == EntityType::StaticHikerActive ||
                      ent->getType() == EntityType::MovingHikerActive || ent->getType() == EntityType::RacingHiker ||
                ...
                if (hiker->getCurState() != EntityAIState::Idle) {
                        if (ent->getType() == EntityType::MovingHikerActive ||
        ...
        if (std::abs(distanceToPlayer) < 1.5)
                chanceSpeedup = 0.5;
