...
TileEntity::TileEntity(const std::weak_ptr<sf::RenderWindow>& window, std::pair<double, double> position,
                       std::pair<double, double> size, std::shared_ptr<sf::Texture> texture, sf::Vector2u texPosition,
                       bool stationary)
    : _window(window), tileSet(texture), stationary(stationary)
{
...
void TileEntity::display() { _window.lock()->draw(*tileRect); }
...
void TileEntity::setStatic(bool _static) { stationary = _static; }
} // namespace turbohikerSFML