...
RacingHiker::RacingHiker(const std::weak_ptr<sf::RenderWindow>& window, const ini::Configuration& configFile)
    : window(window)
{
...
void RacingHiker::display() { window.lock()->draw(*racerRect); }
...
void RacingHiker::setVelocity(const std::pair<double, double>& _velocity) { Entity::setVelocity(_velocity); }
...
void RacingHiker::setOrigin(const std::pair<double, double>& _origin)
{
        ...
        auto convertedOrigin =
            Transformation::convertSizeToPixels(*window.lock(), {_origin.first, getSize().second - _origin.second});
