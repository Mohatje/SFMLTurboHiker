...
namespace turbohikerSFML {
    ...
    void World::handleGameEvent(sf::Event& event) {
        if (event.type == sf::Event::KeyReleased) {
            if (event.key.code == speedToggle) {
                ...
                setYelled(turbohiker::Random::randFloat(0.f, 1.f) < 0.5f);
            }
        }
    }
    ...
    bool World::removeNearestObstacle() {
        auto pPosition = getPlayerPosition();
        double distance = 1000.0;
        for (auto& ent : getEntities()) {
            if (ent->getType() == turbohiker::EntityType::StaticHikerActive) {
                auto entPos = ent->getPosition();
                if (entPos.second < pPosition.second) {
                    continue;
                }

                double tmpDistance = sqrt( pow((pPosition.first - entPos.first), 2) + pow((pPosition.second - entPos.second), 2) );
        ...
        for (auto& ent : getEntities()) {
            if (ent->getType() == turbohiker::EntityType::StaticHikerActive) {
                auto entPos = ent->getPosition();
                if (entPos.second < pPosition.second ) continue;
