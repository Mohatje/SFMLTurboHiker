...
PassingHiker2::PassingHiker2(const std::weak_ptr<sf::RenderWindow>& window, const ini::Configuration& configFile)
    : _window(window)
{
        ...
        std::string txPath =
            configFile["MovingObstacle"]["Texture"].as_string_or_default("./media/Pipoya/Enemy/Enemy 06-1.png");
        ...
        if (!obstacleTexture->loadFromFile(txPath))
                std::cerr << "Cannot load moving enemy texture " << txPath << ". Please check the configuration file."
                          << std::endl;
...
bool PassingHiker2::doTypeSpecificAction() { return isActive(); }
...
void PassingHiker2::display() { _window.lock()->draw(*obstacleRect); }
