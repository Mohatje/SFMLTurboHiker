...
PassingHiker1::PassingHiker1(const std::weak_ptr<sf::RenderWindow>& window, const ini::Configuration& configFile)
    : _window(window)
{
        ...
        if (!(obstacleTexture->getSize().x > 0 && obstacleTexture->getSize().y > 0)) {
                if (!obstacleTexture->loadFromFile(tilePath)) {
                        std::cerr << "Cannot load texture " << tilePath << ". Please check the configuration file."
                                  << std::endl;
                }
        }
...
void PassingHiker1::display() { _window.lock()->draw(*obstacleRect); }
